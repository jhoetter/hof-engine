...
def test_plan_discover_explore_text_then_clarify_tools(monkeypatch) -> None:
    ...
    assert len(finals) == 1
    pds = [e for e in ev if e.get("type") == "plan_discover"]
    assert pds, "explicit plan_discover NDJSON events (additive to phase.discover_phase)"
    assert pds[0].get("subphase") == "explore"
    assert len(calls) == 2
...
def test_provider_error_transient_when_retryable_even_if_category_odd() -> None:
    class OddCategoryError(RuntimeError):
        def __init__(self) -> None:
            super().__init__("m")
            self.provider = "p"
    ...
    assert _provider_error_is_transient_for_log(OddCategoryError())
