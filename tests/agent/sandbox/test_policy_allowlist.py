...
def test_effective_allowlist_terminal_only_strips_domain() -> None:
    p = AgentPolicy(
        ...
        sandbox=SandboxConfig(
            ...
            builtins_when_terminal_only=frozenset({"hof_builtin_server_time"}),
        ),
    )
    eff = p.effective_allowlist()
    assert eff == frozenset({"hof_builtin_server_time", HOF_BUILTIN_TERMINAL_EXEC})
    assert "read_fn" not in eff
    assert "mut_fn" not in eff
    assert not eff.intersection(BUILTIN_AGENT_TOOL_NAMES - {"hof_builtin_server_time"})
...
def test_sandbox_config_env_override_terminal_only(monkeypatch) -> None:
    ...
    monkeypatch.setenv("HOF_SANDBOX_BUILTINS", "hof_builtin_calculate")
    ...
    assert sc.builtins_when_terminal_only == frozenset({"hof_builtin_calculate"})
